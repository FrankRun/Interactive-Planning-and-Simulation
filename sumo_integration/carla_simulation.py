...
from sumo_integration.interaction_planning.interaction_planning import interaction_planning
...
class CarlaSimulation(object):
    ...
    """
    def __init__(self, host, port, step_length,scenario,num,success_rate):
        ...
        # 初始化交互式规控类
        self.interaction_planning = interaction_planning(self.world, scenario, num, success_rate)
        # set collision sensor
        self.if_set_collision_sensor = True
        self.collision_detected = False  # 初始化碰撞检测标志
    ...
    def on_collision(self,event):
        ...
        print(f'Collision with {actor_type} at intensity {intensity}')
         # 碰撞时更新标志
        self.collision_detected = True
    
    def tick(self,ego_id,ego_vehicle):
        """
        Tick to carla simulation.
        """
        time = self.world.get_snapshot().timestamp.elapsed_seconds

        if ego_vehicle and self.if_set_collision_sensor:
            vehicle = self.world.get_actor(ego_id)
            # 创建碰撞传感器并附加到车辆上
            collision_sensor_bp = self.world.get_blueprint_library().find('sensor.other.collision')
            self.collision_sensor = self.world.spawn_actor(collision_sensor_bp, carla.Transform(), attach_to=vehicle)
            self.if_set_collision_sensor = False
            print("Sensor attached to vehicle.")
            # 启动碰撞传感器的监听
            self.collision_sensor.listen(self.on_collision)
            print("Collision detection enabled.")
        
        '''算法运行'''
        collision_flag, exit_flag = False, False
        if ego_vehicle:
            # 碰撞检测
            if self.collision_detected:
                collision_flag = True
            exit_flag,delta_x,delta_y,delta_fi,u,compete_target = self.interaction_planning.run_simulation(ego_id,ego_vehicle,time)
        ...
        if ego_vehicle:
            return collision_flag, exit_flag, delta_x,delta_y,delta_fi,u,compete_target
        else:
            return collision_flag, exit_flag, False,False,False,False,False
