...
class PPO_continuous():
    ...
    def update(self, replay_buffer, total_steps):
        print('{} steps update'.format(total_steps))
    ...
    def save(self, checkpoint_path,frame_id):
        torch.save(self.actor.state_dict(), checkpoint_path+'actor%s.pth'%(frame_id))
        torch.save(self.critic.state_dict(), checkpoint_path+'critic%s.pth'%(frame_id))
    
    def load(self, checkpoint_path,frame_id):
        self.actor.load_state_dict(torch.load(checkpoint_path+'actor%s.pth'%(frame_id), map_location=lambda storage, loc: storage))
        self.critic.load_state_dict(torch.load(checkpoint_path+'critic%s.pth'%(frame_id), map_location=lambda storage, loc: storage))